...
class TransferRepositoryImpl implements ITransferRepository {
  ...
  Future<void> insert(TransferEntity transfer) async {
    ...
    final newToBalance = isToCreditCard
        ? toAccount.balance
    ...
    await _db.transaction(() async {
      ...
      final toRunningBalance =
          isToCreditCard ? -(newToAmountUsed ?? 0) : newToBalance;
      await _ledgerDao.insertEntry(LedgerEntriesCompanion.insert(
        id: const Uuid().v4(),
      ...
      if (isToCreditCard) {
        await _accountsDao.updateAmountUsed(
            transfer.toAccountId, newToAmountUsed!);
      } else {
